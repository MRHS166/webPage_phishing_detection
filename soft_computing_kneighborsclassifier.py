# -*- coding: utf-8 -*-
"""Soft_Computing_kneighborsClassifier.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/11BIH6gQ0O6DU0nCfHdFlGANEtExDsuL0
"""
...
#Sorts features by Mutual Information score in descending order (highest relevance first).
...
model = KNeighborsClassifier(n_neighbors=5)  ### Initializes KNN with k=5
#model =---------------------
...
print(clf_report)
### Creates and plots confusion matrix with class labels.
...
#Calculates fitness of each chromosome by training model on selected features.
#Evaluates accuracy on test set.
#Sorts chromosomes by accuracy (best first).
#Returns sorted scores and chromosomes.
...
# Selects top n_parents chromosomes from the sorted population for next generation.
...
#Creates a child by combining first half of parent 1 + second half of parent 2.
#Adds the new child to next generation population.
...
### Mutates chromosomes by flipping bits at random positions.
#Number of mutations per chromosome = mutation_rate * number_of_features.
...
# Executes the Genetic Algorithm with given parameters.
...
### Creates and plots confusion matrix for GA-selected features predictions
...
# Prints classification report (precision, recall, F1-score) for predictions from PSO-selected features
print("\nClassification Report after using PSO:")
print(classification_report(y_test, pred_pso))

### Prints confusion matrix for PSO predictions.
...
### Plots a bar chart comparing model metrics (Precision, Recall, F1, Accuracy)

### For three cases: Before feature selection, After GA, After PSO
